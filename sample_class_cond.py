...
if __name__ == "__main__":
    ...
    save_result_folder_path = './output/gen_class_cond/' + dm + '/'
    ...
    with torch.no_grad():
        for category_id in valid_category_id_list:
            ...
            for i in range(total//iters):
                full_sample_array_list = model.sample(cond=torch.Tensor([category_id]*iters).long().to(model_device), batch_seeds=torch.arange(i*iters, (i+1)*iters).to(model_device))
